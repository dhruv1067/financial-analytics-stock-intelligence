...
def MACD(dataframe, num_period):
    ...
    fig.add_trace(go.Scatter(x=dataframe['Date'], y=dataframe['MACD Signal'],name='Overbought', marker_color='red', line=dict(width=2, color='red', dash='dash')))
    fig.add_trace(go.Scatter(x=dataframe['Date'], y=dataframe['MACD Hist'],fill='tonexty', name='Oversold', marker_color='#79da84', line=dict(width=2, color='#79da84', dash='dash')))
    fig.update_layout(yaxis_range=[0,100], height=200, plot_bgcolor='white', paper_bgcolor='#e1efff', margin=dict(l=0, r=0, t=0, b=0), legend=dict(orientation='h',yanchor="top", y=1.02, xanchor="right", x=1))
    return fig
...
def Moving_average_forecast(forecast):
    ...
    fig.update_xaxes(rangeslider_visible=True)
    fig.update_layout(height=500, plot_bgcolor='white', paper_bgcolor='#e1efff', margin=dict(l=0, r=0, t=0, b=0), legend=dict(yanchor="top", xanchor="right"))
    return fig